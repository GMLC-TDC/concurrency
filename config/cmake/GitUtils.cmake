...
# clone a git repo into a directory at configure time this can be useful for including
# cmake-library projects that contain *.cmake files the function will automatically init
# git submodules too
#
...
# [GIT_TAG|GIT_BRANCH|GIT_COMMIT  <symbol>] [DIRECTORY                      <dir>]
# [QUIET] )
#
# ARGUMENTS: PROJECT_NAME name of the project that will be used in output variables.
# must be the same as the git directory/repo name
#
# GIT_URL url to the git repo
#
# GIT_TAG|GIT_BRANCH|GIT_COMMIT optional the tag/branch/commit to checkout default is
# master
#
# DIRECTORY optional the directory the project will be cloned into default is the build
# directory, similar to ExternalProject (${CMAKE_BINARY_DIR})
#
# QUIET optional don't print status messages
#
# OUTPUT VARIABLES: <project name>_SOURCE_DIR top level source directory of the cloned
# project
#
# EXAMPLE: git_clone( PROJECT_NAME    testProj GIT_URL https://github.com/test/test.git
# GIT_COMMIT      a1b2c3 DIRECTORY ${CMAKE_BINARY_DIR} QUIET )
#
...
function(git_clone)
    ...
    cmake_parse_arguments(
        ...
        "QUIET" # list of names of the boolean arguments (only defined ones will be
                # true)
        "PROJECT_NAME;GIT_URL;GIT_TAG;GIT_BRANCH;GIT_COMMIT;DIRECTORY" # list of names
                                                                       # of mono-valued
                                                                       # arguments
    ...
    ) # makes var visible everywhere because PARENT_SCOPE wouldn't include this scope

    set(SOURCE_DIR ${PARGS_PROJECT_NAME}_SOURCE_DIR)

    # check that only one of GIT_TAG xor GIT_BRANCH xor GIT_COMMIT was passed
    at_most_one(
        at_most_one_tag ${PARGS_GIT_TAG} ${PARGS_GIT_BRANCH} ${PARGS_GIT_COMMIT}
    )

    if(NOT at_most_one_tag)
        message(
            FATAL_ERROR "you can only provide one of GIT_TAG, GIT_BRANCH or GIT_COMMIT"
        )
    endif()
